$(document).ready(function(){
    ...
    $.active_slider_dots = function(slide_dots){
        ...
        if((current_active_slide_number >= 1) && (current_active_slide_number <= slides_number)){
            ...
            sd.current_active_dot.removeClass('promotions-active-slide-dot');        
            sd.current_active_slide.removeClass('events-active-slide');        
            sd.next_active_dot.addClass('promotions-active-slide-dot');
    ...
    $('.promotions-slider-dot').click(function(){        
        var me = $(this),           
            ...
            slide_dots = {
                ...
                current_active_dot: $('.promotions-active-slide-dot'),              
    ...
    $.set_see_more_hidden = function(param){    
        for(var i=0; i<5; i++){
            var tab, section_length;
            if(i===0){   
                tab = 'promotion_all';
                section_length = $('.'+tab).length;
            }
            else{
                tab = 'promotion_'+i;
                section_length = $('.'+tab).length;
            }         
            if(section_length<5 || param)
                $('#'+tab).addClass('see-more-hidden');     
        }  
    }
    ...
    $.see_more_events = function(me){
        ...
        var data = {  pagination : page },
            service_url = '/motos/promociones/see_more';
        ...
        $.ajax({
            ...
            success  : function(data) {
                ...
                setTimeout(function(){                    
                    ...
                    setTimeout(function(){
                        if(!data.button){
                            $('.see_more').fadeOut(600);
                            
                            var active_tab = $('.tab.active').attr('id');
                            $('#'+active_tab).addClass('see-more-hidden');
                            if(active_tab==='promotion_all')
                                $.set_see_more_hidden(true);
                        }
                    }, 50);
                }, 50);
            },
    ...
    var path = '/motos/images/sections/promotions/gallery/',
        white_path = '/motos/images/sections/promotions/gallery/white_';
            ...
            if($('.'+tab).length>=5){
                ...
                for(var i=0; i<5; i++){
                    if(i===0){
                        ...
                        if($('#promotion_'+i).hasClass('see-more-hidden'))
                            counter++; 
                    }                    
                }                    
    ...
    $('.tab, .section_wrapper.gallery-box-link').click(function(){
        var me = $(this),
            name = me.data('name'),
            type = me.data('type');
        if(name)
            ga('send', 'event', 'Promotions', type, name.replace(/\s+/g,""));
    });

    if(IS_MOBILE){
        //Define the sections size for mobile based on device width

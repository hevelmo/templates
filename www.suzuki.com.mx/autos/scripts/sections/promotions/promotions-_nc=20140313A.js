...
//            eq = $( '#' + id ).parent().parent().index();
    ...
    var $no_promo_phone = $('#no_promo_phone');
    ...
    $no_promo_phone.on('focusout', function(){
        $.validate_input( $no_promo_phone );
    });
    $.no_promo_join_form = function(){

        var form_errors = 0;
        if( !$.validate_input( $no_promo_phone ) ){
            form_errors++;
            $no_promo_phone.focus();
        }

$( document ).ready( function(){
    ...
    var map;
    var map_markers = [];
    var script_url = '/autos/services/concessionaires' ;
    var title_box = null;
    ...
    $.open_concessionaire_by_key = function( key , change ){
        ...
        if( change ){
            try{
                window.history.pushState( null , cc.name, "/autos/concesionarias/suzuki-" + key );
            }catch( e ){ }
        }
        ...
        $('#concessionaire-image').attr({
            alt     : cc.name,
            src     : 'http://cdn.ktcagency.com/autos/images/sections/concessionaires/previews/' + cc.key + '-big.jpg',
            title   : cc.name
        });
        if (IS_MOBILE) {
        ...
        $.ajax({
            success: function( data ){
                ...
                var icon_latLon, conce, conce_select_html,
                    icon_base   = 'http://' + location.host +'/autos/images/sections/concessionaires/',
                ...
                var icon_options = {
                    boxStyle: {
                        background  : "url('/autos/images/sections/concessionaires/marker-active-bg.png') no-repeat",
                        height      : "67px",
                        width       : "250px"
                    },
                    closeBoxURL: '',
                ...
                for( i1 in concessionaires ){
                    ...
                    conce_select_html += '<option value="' + conce.key + '" data-metadata="'+ conce.address+'">' + conce.name + ", " + conce.state+ '</option>';

...
function SearchForm(_config) {
	...
	this.customSelectFields = {
		...
		'bauart': {
			updateOptions: function (elem, json, q) {
				...
				if (selectedIndex === 0) { elem.selectedIndex = 0; } // Fix WebKit selection bug
			...
			updateOptions: function (elem, json, q) {
				...
				for (var i in manufacturers) {
					if (manufacturers[i].used) {
						...
						console.log(manufacturers[i].name);
					}
				}
	...
	this.getQuery = function (preProcess, which) {
		...
		var query = {
			...
			'allow_no_price': '1',
			'noAvTs': '1',
			'show_extras': '29,seven_seats,10,420,44,13,73,automatic,25,545,50,561,441,102,421,35,37,432,578,67,442,414,522,hybrid,418,429,8,540,99,metallic,has_pic,vat,36,51,340,641,423,private_ad,614,64,38,sideairbags,53,39,60,31,62,148,46,47,accident,611,607,609,577,371,42,43,hasOffer,625,796',
			'locale': _this.locale,
	...
	this.refresh = function (which) {
		...
		httpRequest(_this.domain + _this.interface + getQueryString(query), function (json, details) {
			...
			_refreshing = false;
			if (_this.debugging) { console.log('AJAX Error\n\n' + e.msg + (e.constructor ? '\n\nConstructor: ' + e.constructor : '') + (e.sys_msg ? '\n\nSystem message:\n' + e.sys_msg : '') + (e.url ? '\n\nURL: ' + e.url : '') + (e.responseText ? '\n\nResponse:\n' + e.responseText : '') + (e.fileName ? '\n\nFile name: ' + e.fileName : '') + (e.lineNumber ? '\n\nLine: ' + e.lineNumber : '') + (e.stack ? '\n\nStack: ' + e.stack : '')); }
		}, { "cache":true });

...
_bubble.prototype.show = function(className, message) {
    ...
    if(push[1]){
        $alert_url .attr( 'href', "#" + push[1] );
        $alert_url .attr( 'title', message.url );
      ...
      try{
        ...
        console.log('throw dummy-alert');
      }

    }
    $alert_url.bind("click", function (e) {
        e.preventDefault();
        console.log("click");
        var razon = $(this).attr("href").substring(1);
        ga('send', 'event', 'Tooltip', 'Clic_popup', razon);
        window.location = $(this).attr("title");
    });
    this.el.find('p.alert-message').text(message.text);
    this.el.attr('class', 'dummy-alert show');

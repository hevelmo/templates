...
myConfig = {
	...
	'updatefinancebutton': function (){
		...
		var url_finance = 'http://'+bypass.agencia.dominio1+'/financiamiento-'+this.slug+'?my=MY17&dc='+this.dc+'&r='+this.rims_id+'&a='+this.accesories_id+'&c='+this.color_id+'&v='+this.version_idx+'&ver=2&origen=config';
...
$( "#sendemail" ).click(function() {
  ...
  $('#imgminicar').attr('src',myConfig.exterior_imagex+'/02.jpg');
...
$(function() {
    ...
    setGlobalVars = function(){
    	...
    	// Si es movil aplicar resize a los exteriores dinamicamente

    	if(/Android|webOS|iPhone|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ) {
			/*frame = base_url+'system/img/360/MOVIL/EXTERIORES/'+modelo+'/'+version+'/'+color+'/'+rin+'/'+acc+'/'+'0#.jpg';
			...
			frame_alt = base_url+'system/img/360/MY17/'+modelo+'/'+version+'/'+color+'/'+rin+'/'+acc+'/';
			frame = base_url+'system/image-resize.php?&w=550&src='+encodeURIComponent(frame_alt)+'0#.jpg';
			myConfig.exterior_imagex = base_url+'system/img/360/MY17/'+modelo+'/'+version+'/'+color+'/'+rin+'/'+acc;
			rbase = base_url+"system/img/360/";

		}else{
			frame = base_url+'system/img/360/MY17/'+modelo+'/'+version+'/'+color+'/'+rin+'/'+acc+'/'+'0#.jpg';
			myConfig.exterior_imagex = base_url+'system/img/360/MY17/'+modelo+'/'+version+'/'+color+'/'+rin+'/'+acc;
			rbase = base_url+"system/img/360/";
		}
    }
	...
	switch_360 = function(tipo,folder,texto){
        ...
        /*var frame = base_url+'system/img/360/MY17/'+modelo+'/'+version+'/'+color+'/'+rin+'/'+acc+'/'+'0#.png';*/
        myConfig.exterior_imagex=base_url+'system/img/360/MY17/'+modelo+'/'+version+'/'+color+'/'+rin+'/'+acc;
 	...
 	mostrarInteriores = function(image1,image2,isDefault){
 		...
 		// Si es movil aplicar resize a los interiores dinamicamente

 		if(/Android|webOS|iPhone|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ) {

 			var urlo = base_url+'system/image-resize.php?&w=720&src='+encodeURIComponent(rbase+image1);
	
 			var im1 = '<div style="background-image:url('+urlo+');" />';
 			var im2 = '<div style="background-image:url('+urlo+');" />';
	...
	$('#image').reel({
		images:      base_url+'system/img/360/MY17/'+modelo+'/'+version+'/'+color+'/'+rin+'/'+acc+'/0#.jpg',
		frames:      8,
		frame:       2, 
		loops:       true,
		speed:       0,

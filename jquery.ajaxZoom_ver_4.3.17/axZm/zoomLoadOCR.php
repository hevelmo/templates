<?php
/**
* Plugin: jQuery AJAX-ZOOM, loadXML.php
...
$ocrFilesPath = str_replace(array('http://', isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : '', isset($_SERVER['SERVER_NAME']) ? $_SERVER['SERVER_NAME'] : '', isset($_SERVER['DOCUMENT_ROOT']) ? $_SERVER['DOCUMENT_ROOT'] : ''), '', $ocrFilesPath);
...
if (substr($ocrFilesPath,0, 1) != '/'){
	...
	$fromPath = str_replace(array('http://', isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : '', isset($_SERVER['SERVER_NAME']) ? $_SERVER['SERVER_NAME'] : '', isset($_SERVER['DOCUMENT_ROOT']) ? $_SERVER['DOCUMENT_ROOT'] : ''), '', array_shift(explode('?',$_SERVER['HTTP_REFERER'])));
			...
			while ($reader->read()) {
				...
				if($reader->nodeType == XMLREADER::ELEMENT && $reader->name == 'String'){
					...
					if (mb_strlen($word) > 4){
						...
						if (!is_array($arrayWords[$word])){$arrayWords[$word] = array();}
						$pos = array();
						$pos['P'] = $nn; // page
